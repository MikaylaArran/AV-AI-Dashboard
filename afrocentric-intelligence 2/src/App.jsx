...
export default function App() {
  ...
  async function fetchIntelligence(queryObj, force = false) {
    ...
    try {
      ...
      const timeLabel = parsed.cachedAt
        ? `${new Date(parsed.cachedAt).toLocaleDateString()} ${new Date(parsed.cachedAt).toLocaleTimeString()}`
        : new Date().toLocaleTimeString();
      setFetchedAt(f => ({ ...f, [queryObj.id]: timeLabel }));
